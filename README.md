# PacMan web game on Akash DeCloud deployment guide
![title picture](https://github.com/yuravorobei/pacman-on-akash-guide/blob/main/media/main.png)

  In this guide for the "Akashian Challenge: Phase 3 Start Guide" I'll show you how to deploy the [PacMan web game](https://en.wikipedia.org/wiki/Pac-Man) on the [Akash Decentralized Cloud (DeCloud)](https://akash.network/).  
...
# 1. Dockerizing
The Akash works with a Docker images, so the first thing we need to do to deploy is to get the Docker image of the application. If you already have the image or know how to get it, you can skip this step.


### 1.1 Source code
First, you need to get the source code of the PacMan game on your computer. I will use accurate remake of the original game from [this github repository](https://github.com/masonicGIT/pacman).
To get the code, clone the repositiory:
  ```bash
  $ git clone https://github.com/masonicGIT/pacman.git
  $ cd pacman
  ```
...
This project does not require installation of dependencies or assembly and is immediately available as a static resource. You can open the `index.htm` file in your browser and the game will start. You need to organize hosting for this assembly. I recommend using Nginx as a web server for the production version of the application. Nginx is a fast web server that has the best performance in this case and supports routing, static content, etc., in an objectively faster time to provide the greater user experience.
...
As a base image used here Alpine-version of `nginx: alpine`, it is a lightweight image that will save space. Then specify the working directory (i use `/app`) and copy all the files and folders of the project there. After that, copy the `ngix.conf` file, which will be created in the next step of the tutorial, to the Nginx configuration directory. Then we open port `80`, you can specify any other. The container will wait for connections on this port to web-server and the same port will need to be specified further in the web server configuration. The last line of the file is used to start Nginx in foreground so that the Docker container does not close immediately after starting.
...
To build the Docker image run the following command:
  ...
  ```
Any other name can be used here instead of `pacman`, most importantly, do not forget to also change this name in the following commands. Waiting for the completion of the function.
...
Then find the created image ID using `docker images` command:
  ...
  pacman              latest              d9f64bb5ca15        3 minutes ago       46.8MB
  ```
The ID of my image is `d9f64bb5ca15`.  
Then tag your image with the following command, replace `<image_id>` with your image ID and replace `<your_hub_username>` with your Docker Hub usesrname. Any name can be used instead of `pacman-web`:
  ```bash
  $ docker tag <image_id> <your_hub_username>/pacman-web
  ```
Push your image to the Docker Hub repository with the following command:
  ```bash
  $ docker push <your_hub_username>/pacman-web
  ```
Waiting for loading.
Dockerization done!
...
The `services.web.images` field is the name of your published Docker image. In my case it is `yuravorobei/pacman-web`.
...
You should see a response similar to:
  ```json
  {
    "services": {
      "web": {
        ...
        "total": 1,
        "uris": [
          "80i9jda3b9ddr6cqeg72j4o938.provider1.akashdev.net"
        ],
  ...
  ```
You can access the application by visiting the hostnames mapped to your deployment. In above example, its http://80i9jda3b9ddr6cqeg72j4o938.provider1.akashdev.net. Following the address, make sure that the application works:
![result1](https://github.com/yuravorobei/pacman-on-akash-guide/blob/main/media/deployed.png)
...
```
[web-59c66c589c-llkcv] /docker-entrypoint.sh: /docker-entrypoint.d/ is not empty, will attempt to perform configuration
[web-59c66c589c-llkcv] /docker-entrypoint.sh: Looking for shell scripts in /docker-entrypoint.d/
[web-59c66c589c-llkcv] /docker-entrypoint.sh: Launching /docker-entrypoint.d/10-listen-on-ipv6-by-default.sh
[web-59c66c589c-llkcv] 10-listen-on-ipv6-by-default.sh: Getting the checksum of /etc/nginx/conf.d/default.conf
[web-59c66c589c-llkcv] /docker-entrypoint.sh: Launching /docker-entrypoint.d/20-envsubst-on-templates.sh
[web-59c66c589c-llkcv] /docker-entrypoint.sh: Configuration complete; ready for start up
```
